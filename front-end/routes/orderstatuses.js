...
router.get('/', isAuth, authIsAdmin, async function(req, res, next) {
  // #swagger.summary = 'View order statuses'
  let token = req.cookies.token
  console.log("-- TOKEN USED:" + token)
  try {
    const response = await fetch('http://localhost:3001/orderstatuses/', {
      method: 'GET',
      credentials: 'include',
      headers: {
    ...
    res.render('orderstatuses', { orderstatuses: data.data.orderstatuses || [] });
  } catch (error) {
